...
# ---------------------------------------------------------------------------
# Detailed mode toggle — OUTSIDE the form so it shows fields immediately
# ---------------------------------------------------------------------------
st.subheader("Entry Mode")
detailed_mode = st.toggle("Detailed Hand (full street-by-street)", value=False)
if detailed_mode:
    st.caption("Full detail mode — all streets and hole cards.")
else:
    st.caption("Quick mode — key decision point and tags only.")
...
with st.form("hand_entry_form", clear_on_submit=False):
    ...
    # --- Detailed fields (only shown when detailed_mode is on) ---
    if detailed_mode:
        st.subheader("Full Hand Detail")

        col7, col8 = st.columns(2)
        with col7:
            hero_cards = st.text_input("Your Hole Cards", placeholder="e.g. AsKh")
        with col8:
...
if submitted:
    ...
    st.caption(
        f"Sample size: {profile['hand_count']} hand(s) | "
        f"Confidence: {'Low' if profile['hand_count'] < 5 else 'Medium' if profile['hand_count'] < 15 else 'High'}"
    )
